...
def __get_input(
    ...
    return (
        ...
        if session
        else prompt(message=message, multiline=False)
    )
...
if __name__ == "__main__":
    ...
    try:
        while True:
            print()
            user_prompt = __get_input("Input: ", session=prompt_session, completer=completions)
            console.print()
            if user_prompt == "!exit":
                break
            ...
            console.print("Google Bard:",Markdown(response["content"]), end=' ')
            print()
